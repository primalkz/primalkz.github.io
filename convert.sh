...
preview=$(head -n 10 "$input_file" | sed '/^<!--/,/^-->/d' | sed '/^#/d' | sed '/^$/d' | head -n 2 | tr '\n' ' ' | sed 's/^[[:space:]]*//;s/[[:space:]]*$//' | cut -c1-150)
main_title=$(head -n 10 "$input_file" | sed '/^<!--/,/^-->/d' | grep "^# " | head -n 1 | sed 's/^# //')

if command -v pandoc >/dev/null 2>&1; then
    html_content=$(sed '/^<!--/,/^-->/d' "$input_file" | pandoc --from markdown --to html --wrap=preserve)
else
    html_content=$(sed '/^<!--/,/^-->/d' "$input_file" | sed 's/$/<br>/' | sed 's/^#\+ \(.*\)/<h\1>\1<\/h\1>/' | sed 's/\*\*\([^*]*\)\*\*/<strong>\1<\/strong>/g' | sed 's/\*([^*]*)\*/<em>\1<\/em>/g')
fi
...
<meta property="og:url" content="/posts/$(basename "$output_file")" />
